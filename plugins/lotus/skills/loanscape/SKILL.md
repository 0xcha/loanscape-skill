...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/brief.mjs" --json
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/brief.mjs" --ladder <row number> --shock <percent>
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/core/brief.mjs" --move <row number>
```
...
- "Can this run every morning?", "send me this daily": yes, and the user never touches a terminal. Route by where you are:
  ...
  3. **Neither**: run `sh "${CLAUDE_PLUGIN_ROOT}/core/install-routine.sh" --at 08:00` (their hour). It schedules the brief script locally, pops a notification with the first line each morning, and keeps the full text in `~/.loanscape/brief.log`. Confirm in one line with the time and the log path. "Stop it": same script with `--remove`.
